    ...
    var aA=new Array();
    var timerArray;
    var image_path = "images/thumbnails/Design/";
    var galleryarray=new Array();
    var gallery_width = 700;
	...
	function init(){
			galleryarray = [image_path + "1.jpg",image_path + "2.jpg",image_path + "3.jpg",image_path + "4.jpg",image_path + "5.jpg"];
			document.getElementById('galleryContainer').innerHTML = "";
        	myImagesTotal = galleryarray.length;
			x_spacing = parseInt((gallery_width - image_width)/2);
			y_spacing = parseInt((gallery_height - image_height)/2);
			x_movement = parseInt(image_width*0.6);
			y_movement = parseInt(image_height*0.3);
            var ni = document.getElementById('galleryContainer');
            var i=0;
		...
		newdiv.style.left = x_spacing + 'px';
		...
		ni.appendChild(newdiv);
            ...
            updateIndex();
            window.clearInterval(timerArray);
            timerArray = window.setInterval("animate();",50);
    }
        ...
        function animate(){
            ...
            var x,y,scl,a,scldW,scldH,imgH,imgW,divL,divT,difL,difT,difH,difW;
                for (i=0;i<galleryarray.length;i++)
                {
                    ...
                    difW = parseInt(Math.abs(imgW - scldW)*.2);
                    difH = parseInt(Math.abs(imgH - scldH)*.2);
                    divL = parseInt(tmpDiv.style.left);
                    divT = parseInt(tmpDiv.style.top);
                    difL = parseInt(Math.abs(divL - x)*.2);
                    difT = parseInt(Math.abs(divT - y)*.2);
